...
function renderSales(sales) {
  ...
  if (!sales.length) {
    salesBody.innerHTML = '<tr><td colspan="8">Todavía no hay cuentas liquidadas.</td></tr>';
    return;
  }
  ...
  salesBody.innerHTML = sales.map((sale) => `
    <tr>
      ...
      <td class="amount">${formatMoney(sale.discount)}</td>
...
async function loadSales() {
  ...
  try {
    ...
    renderSales(sales);
    setMessage(`Mostrando ${sales.length} cuenta(s).`);
  } catch (error) {
