 ...
 *   - Header date/time (Task 10.2): displays current time, updates every second
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Task 10.2 — Display current date/time and update every second
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (!paymentOpen && event.ctrlKey && event.key === "Backspace") {
      event.preventDefault();
      if (CartStore.items.length > 0 && window.confirm("¿Limpiar la cuenta?")) {
        CartStore.clearCart();
      }
      return;
    }

    if (!paymentOpen && event.key === "+") {
      event.preventDefault();
