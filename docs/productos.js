...
const BACKEND_API_BASE_URL = window.KIRO_BACKEND_API_BASE_URL || "http://localhost:8081";
...
function openForm(mode = "create") {
    ...
    if (mode === "create") {
        formTitle.textContent = "Crear artículo";
    }
}
...
function fillForm(producto) {
    ...
    formTitle.textContent = `Editar artículo #${producto.id}`;
...
async function loadProductos() {
    ...
    try {
        ...
        paginationInfo.textContent = `Página ${data.page} de ${data.totalPages} — ${data.total} artículos`;
    } catch (error) {
        setMessage("error", error.message || "No fue posible cargar artículos");
    }
}
...
async function loadBackendProductos() {
    try {
        ...
        paginationInfo.textContent = `Página ${page.page} de ${page.totalPages} — ${page.total} artículos`;
    } catch (error) {
        setMessage("error", error.message || "No fue posible cargar artículos desde AWS");
    }
}
...
window.eliminarProducto = async function(id) {
    const confirmado = confirm("¿Seguro que deseas eliminar este artículo?");
    ...
    try {
        ...
        setMessage("success", data.message || "Artículo eliminado correctamente");
        resetForm();
        closeForm();
        await loadProductos();
    } catch (error) {
...
window.toggleEstadoProducto = async function(id) {
    if (USE_BACKEND_PRODUCTS) {
        setMessage("error", "En modo AWS esta pantalla crea y edita artículos; eliminar/restaurar no está disponible en el backend SAM actual.");
        return;
    }
    ...
    const confirmado = confirm(`¿Seguro que deseas ${accion} este artículo?`);
    if (!confirmado) return;
...
productoForm.addEventListener("submit", async (e) => {
    ...
    try {
        ...
        setMessage("success", data.message || (isEdit ? "Artículo actualizado" : "Artículo creado"));
        resetForm();
        closeForm();
        await loadProductos();
    } catch (error) {
