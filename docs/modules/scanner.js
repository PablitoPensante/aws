...
async function lookupBarcode(barcode) {
  ...
  try {
    ...
    products = (Array.isArray(data) ? data : [data]).filter(Boolean).map(normalizeProduct);
  } catch {
    try {
      const allProducts = await apiFetch("/api/products");
      products = (Array.isArray(allProducts) ? allProducts : []).filter(Boolean).map(normalizeProduct);
    } catch {
  ...
  if (!match) {
    showError(`No existe un artículo con código ${lookupCode}.`);
    return;
  }
