...
function onCashAmountInput() {
  ...
  if (cashAmount >= grandTotal) {
    ...
    changeDueEl.textContent = "$0.00";
    changeDueEl.classList.remove("price--success");
    showError("El efectivo recibido no alcanza para liquidar la cuenta.");
    setConfirmEnabled(false);
  }
}
...
function onMixedAmountInput() {
  ...
  if (total >= grandTotal) {
    ...
    mixedChangeEl.textContent = "$0.00";
    mixedChangeEl.classList.remove("price--success");
    showError("El total recibido no alcanza para liquidar la cuenta.");
    setConfirmEnabled(false);
  }
}
...
async function onConfirmPayment(openReceiptFn) {
  ...
  const transaction = {
    ...
    orderDiscount: CartStore.orderDiscount,
    subtotal: CartStore.subtotal,
  ...
  try {
    ...
    showError(error?.message ?? "No fue posible liquidar la cuenta en el backend.");
    confirmBtn.disabled = false;
    confirmBtn.textContent = "Confirmar liquidación";
    return;
  }
