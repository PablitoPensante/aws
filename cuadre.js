...
function renderSalesTable(sales) {
  ...
  if (!sales.length) {
    salesBody.innerHTML = '<tr><td colspan="6">No hay cuentas para este filtro.</td></tr>';
    return;
  }
...
function renderClosingStatus(difference) {
  ...
  if (difference === null) {
    closingStatus.textContent = "Ingrese el efectivo contado para calcular el cierre.";
    return;
  }
...
async function loadSales() {
  ...
  try {
    ...
    renderClosing();
    setMessage(`Cierre calculado con ${allSales.length} cuenta(s) recientes.`);
  } catch (error) {
    setMessage(error?.message || "No fue posible cargar las cuentas.", true);
  }
}
